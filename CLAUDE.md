...
## Design system
- **Palette:** warm ivory, espresso and a little muted gold. Blue (`#1B5FE6`) is kept to a minimum: the logo, the "Get an offer" button in the header, keyboard focus rings, the reading-progress line under the header, which the owner asked to be a faint deep blue (`--progress`, `rgba(38, 84, 196, 0.55)`), and the hero's italic second line in the same deep blue (`--hero-blue`: `#2654C4` in light mode, lifted to `#4470E0` in dark mode so it stays readable on black). The owner asked for less blue, so don't add it back.
